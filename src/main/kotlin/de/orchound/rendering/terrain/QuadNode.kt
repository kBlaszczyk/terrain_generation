...
class QuadNode(
	private val drawDeterminer: DrawDeterminer, val camera: Camera, val material: TerrainMaterial,
	...
	init {
		...
		if (width > 4f) {
			for (i in 0 .. 3) {
				...
				val child = QuadNode(
					drawDeterminer, camera, material, translation.add(xOffset, yOffset, Vector2f()),
	...
	fun draw() {
		if (children.isEmpty() || drawDeterminer.determineNodeRendering(this)) {
			...
			material.setTessellationOrigin(tessellationOrigin)
			material.setTessellationWidth(tessellationWidth)
